...
%They prepare several translucent colored shapes,
%which are optimized by a loss function. The optimized shapes are
%deployed on the camera's lens by adversaries to attack autonomous
%driving systems.
...
by optimize the objective function: 
\begin{align}
	\min \Vert f_{mde}(I^{adv}) - D^{*} \Vert
\end{align}
where $||\dot||$ denotes a metric for distance.
...
%The mentioned attack methods need access to the objects they 
%attacked. The advantage is that the adversarial examples are more 
%inconspicuous. However, it is hard to deployment in the 
%physical world. 
%To this end, Zolfi et al.~\cite{Zolfi_2021_CVPR} proposed 
%a contactless attack method. 
%They prepare several translucent colored shapes,
...
\footnote{Eq.~\ref{define} is a simplified definition of $I^{adv}$.
the concrete definition will be given in Sec.~\ref{Patch Design}.}
...
Denote the victim MDE model as $f_{mde}$, 
it was trained on the training set 
$\mathbb{T}={(I^{H \times W \times 3}, D^{* H \times W})}$. 
We aim to derive the optimal adversarial examples in an optimization manner:
\begin{align}
	\mathop{\mathrm{argmim}}\limits_{I^{adv}}~~{\mathcal{L}(I^{adv})},
	\label{objective}
\end{align}
where $\mathcal{L}$ represents the objective function, which
will be designed under two considerations: 
...
\begin{figure}
	\centering
	\includegraphics[width=0.2\textwidth]{imgs/patch.png}
	\caption{Our patch selected from QuickDraw.}
	\label[]{patch}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.45\textwidth]{imgs/mask.pdf}
	\caption{Illustration of our mask $M$, with which we can 
	\label[]{mask}
	generate adversarial examples through $M \odot I$.}
\end{figure}
...
We select a drawing from QuickDraw~\cite{quickdraw},
a collection of 50 million hand-writing drawings across 345 categories,
as our patch$\delta$. The drawing is shown in Fig.~\ref{patch}.
...
Fig.~\ref{mask} shows our mask and the representation of our
...
location. We design $\mathcal{L}_{sensitive}$ and search the 
sensitive location by maximize the function: 
\begin{align}
...
and $n$ is the number of pixels in patch $\delta$.
...
the target coordinates $G'_i$ 
are continuous values. In other words, the value
of the target pixels are computed by a sampling method. 
We adopt bilinear interpolation sampling to determine the value of 
the target pixel, which is proved 
...
To avoid patches overflowing from clean images, $t_x$ and $t_x$
...
NYU depth v2 consisits of 1449 densely annotated depth and 
RGB images pairs,
