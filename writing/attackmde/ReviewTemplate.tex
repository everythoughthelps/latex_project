...
When we alter the appearance of the patch, we followed 
...
Using the example shown in fig1 can clearly 
illustrate what this means.
\begin{figure}
\includegraphics[width=1\linewidth]{imgs/Perspective.jpg}	
\end{figure}
So in this phase, we utilize vanishing points for better 
visual quality. We first detect straight lines in the attacked 
scene with \textit{Hough} transform, then we compute the vanishing 
point using lines with a longer length.
...
inconspicuous.
Based on these three factors, we design three optimization terms in
our objective formulation, which are summed up in a linear manner:
\begin{equation}
\begin{aligned}
	&\mathcal{L}_{total} = \lambda_1\mathcal{L}_{attack} +
	\lambda_2\mathcal{L}_{perspective} +
	\lambda_3\mathcal{L}_{blank}\\ 
	&\sum_{i}{\lambda_i} = 1
\end{aligned}
\end{equation}
...
To obtain best attack effect means, that is, 
to maximize the error between prediction maps and annotated maps:
\begin{align}
\mathcal{L}_{attack} = \Vert y - y^{*} \Vert_{p}
\end{align}
We use $L_1, L_2$ and Huber loss in our subsequent experiments.

\paragraph{Visualization Quality}
		\begin{align}
			\label[]{perspective}
		\mathcal{L}_{perspective}
		\end{align}

For better inconspicuousness, we camouflage the patch 
as graffiti, which can be easily found on white and 
blank walls. We design Blank loss to help the patch to find
blank position in the area:
	\begin{align}
		\label[]{blank}
	\mathcal{L}_{blank} =\sqrt{\partial_x^2 (m \odot I) + \partial_y^2 (m \odot I) }
 	\end{align}
Besides, we designed white loss to make the 'graffiti' more likely to be sticked 
in the white area:
\begin{align}
	\mathcal{L}_{white} = \min (1-\sum m \odot I, 0)
\end{align}
...
which area of the adversarial example is more inconspicuous 
and how to make the patches more 
inconspicuous. We first show our attack effect in 
Sec~\ref{Visualization Attack Results} and 
...
Blank (eq~\ref{blank}) and Perspective (eq~\ref{perspective}) 
...
\begin{figure*}
	...
	Patch \ding{176} is a black patch filled with 0 and patch \ding{177} is a 
	white patch filled with 1.
