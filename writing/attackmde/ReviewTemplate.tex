...
Therefore, when select patches, 
simple line-drawn patches are preferred.
We select a patch from QuickDraw~\cite{quickdraw},
a collection of 50 million hand-writing drawings across 345 categories,
to generate $I^{adv}$.

As you can see from Fig.~\ref{patch_mask}, the patch is simple line-drawn,
...
In doing so, we define a mask $M \in {0,1}^n$
Thus, to reduce modifications to $I$, $I^{adv}$ can be represented by:
We use following equation denote our adversarial examples:
\begin{align}
	I^{adv} = M \odot I 
\end{align}
